...
#define USAGE "Usage: bchunk-bin2iso [-v] [-r] [-p (PSX)]\n" \
        "         <image.bin> <image.iso>\n" \
	"Example: bchunk foo.bin foo.iso\n" \
	"  -v  Verbose mode\n" \
...
void parse_args(int argc, char *argv[])
{
	...
	while ((s = getopt(argc, argv, "t:vp?hr")) != -1) {
		switch (s) {
			case 'r':
				raw = 1;
				break;
...
int writetrack(FILE *bf, struct track_t *track, FILE *f)
{
	char *fname;
	char buf[SECTLEN+10];
...
int main(int argc, char **argv)
{
	...
	if (!((isof = fopen(isofile, "wb")))) {
		fprintf(stderr, "Could not create ISO %s: %s\n", isofile, strerror(errno));
		return 2;
	}
	...
	/*
	if (!fgets(s, CUELLEN, isof)) {
		fprintf(stderr, "Could not read first line from %s: %s\n", isofile, strerror(errno));
		return 3;
	}
	
	while (fgets(s, CUELLEN, isof)) {
	...
	*/
	...
	track->start = 0;
	track->startsect = 0;
	track->extension = strdup("iso");
	track->modes = strdup("MODE2/2352");
	if (raw) {
		/* Raw MODE2/2352 */
		track->bstart = 0;
		track->bsize = 2352;
	...
	if(index_ts)
	{
		track->startsect = time2frames(index_ts);
		track->start = track->startsect * SECTLEN - 8;
		if (verbose)
			printf(" (startsect %ld ofs %ld)", track->startsect, track->start);
	}
